...
static u8 m_wait_timer_tick = 0;

static u32 m_past_tick = 0;

u32 system_past_tick = 0;
...
void timer_create(u8 *timer_index, app_timer_timeout_handler_t timerout_hander)
{
	...
	else
	{
		*timer_index = m_current_timer_num;
		...
		m_timer_manager[m_current_timer_num].timer_threshold = 0;
...
void timer_start(u8 timer_index, u32 duration)
{
	...
	m_timer_manager[timer_index].timer_threshold = duration;
